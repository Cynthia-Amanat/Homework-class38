...
function computeEarnings (mondayTasks,hourlyRate) {
  ...
  const personSalaries = mondayTasks.map(({duration}) => hourlyRate * duration / 60)
  const totalAmount = personSalaries.reduce((acc , curr) => acc + curr) 
